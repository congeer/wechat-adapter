...
export default class XMLHttpRequest {
  ...
  send(data) {
    ...
    if (!this.url.match(/^https?/)) {
      ...
      this.status = 200;
      readFile(this.url, this.responseType === 'text' ? 'utf-8' : this.responseType)
        .then((res) => {
        ...
        .catch((err) => {
          this.response = this.responseText = err;
          this.emit('error');
        });
    } else {
      wx.request({
        ...
        success: (info) => {
          ...
          this.response = this.responseText = info.data;
          this.emit('load');
          this.emit('readystatechange');
        },
        fail: (info) => {
          this.readyState = XMLHttpRequest.DONE;
          this.status = info.statusCode;
          this.response = this.responseText = info;
          this.responseHeader = info.header;
          this.emit('error');
          this.emit('readystatechange');
        },
